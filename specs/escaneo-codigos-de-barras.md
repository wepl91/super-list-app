...
> **Decisión (2026-09-09)**: la feature fue **descartada**. El escaneo era sólido
> (v3, implementado y con tests), pero **Open Food Facts no resolvía la mayoría
> de los códigos del mercado local (779 Mercosur)** y la experiencia quedaba en
> mano del usuario ("no se pudo encontrar el producto"). Se cierra el PR #45 sin
> mergear. Este spec queda archivado como referencia para una futura iteración
> **más escalable** (p. ej. fuentes/lookups múltiples, enfoque local-first con
> sugerencias de productos frecuentes del usuario en lugar de depender de una API
> única).
